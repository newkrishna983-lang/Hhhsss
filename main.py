...
import core as helper          # ✅ added import
from core import *             # (optional, but helper is now used)
...
# ✅ Updated authorization – only owner and sudo users
...
image_urls = ["https://files.catbox.moe/v9z1n7.jpg"]
...
async def ytplaylist_to_txt(client: Client, message: Message):
    # ✅ fixed: use from_user.id for owner check
...
async def upload(bot: Client, m: Message):
    # ✅ fixed: use from_user.id for authorization
    if not is_authorized(m.from_user.id):
    ...
    try:
        ...
        arg = 1
    # ✅ fixed: use parsed arg for count, don't re-parse
    count = arg
    ...
    if len(links) == 1:
        ...
        count = arg  # ✅ already parsed above
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            # ... (many URL transformations, kept unchanged) ...
            ...
            try:
                ...
                cpvod = f'**🎬 Vɪᴅ Iᴅ : {str(count).zfill(3)}.\n\n\nTitle : {name1}.({res}).mkv\n\n\n🔗𝗩𝗶𝗱ᴏ 𝗨𝗿𝗹 ➤ <a href="{url}">__Click Here to Watch Video__</a>\n\n📚 Bᴀᴛᴄʜ Nᴀᴍᴇ : {b_name}\n\n📇 ᴇxᴛʀᴀᴄᴛᴇᴅ ʙʏ : {CR}**'
